...
(function (global) {
  ...
  SurveyChat.prototype._wire = function () {
    ...
    this._wired = true;
    if (this.o.sendBtn) this.o.sendBtn.addEventListener("click", function () { self._handle(); });
    if (this.o.input) this.o.input.addEventListener("keydown", function (e) {
  ...
  SurveyChat.prototype._bot = function (text, cb) {
    ...
    var row = this._botRow();
    var t = el("div", "typing"); t.appendChild(el("i")); t.appendChild(el("i")); t.appendChild(el("i"));
    row.appendChild(t);
    this.o.log.appendChild(row); this._scroll();
    setTimeout(function () {
      if (g !== self.gen) { row.remove(); return; }
      t.remove();
      var bub = el("div", "bub bot", text);
      row.appendChild(bub);
      self._lastBotBub = bub;
      self._scroll();
      if (cb) cb();
    }, 420);
  };
  SurveyChat.prototype._nag = function (text) {
    ...
    setTimeout(function () {
      ...
      row.appendChild(self._nagEl);
      self._scroll();
    }, 420);
  };
  SurveyChat.prototype._me = function (text) {
    var row = el("div", "chat-row me");
    row.appendChild(el("div", "bub me", text));
    this.o.log.appendChild(row);
    this._scroll();
  };
  ...
  SurveyChat.prototype._input = function (enabled, ph) {
    if (!this.o.input || !this.o.sendBtn) return;
  ...
  SurveyChat.prototype._chips = function (options, onPick, chipOpts) {
    ...
    var host = this._lastBotBub || this.o.log;
    host.appendChild(box); this._scroll();
    box.tabIndex = -1;
